...
var hexRegex = /^#[a-z0-9]{6}$/;

var colorSchema = _joi2.default.object().keys({
  hex: _joi2.default.string().regex(hexRegex).required(),
  rgb: _joi2.default.array().items(_joi2.default.number()).length(3).required()
});

var colorsSchema = _joi2.default.object().pattern(/[a-zA-z]/, colorSchema).required();

var mainSchema = _joi2.default.object().length(30).pattern(/[A-Z]{3}/, _joi2.default.object().keys({
  fullName: _joi2.default.string().required(),
  mainColor: colorSchema.required(),
  colors: colorsSchema
}));

var colorsListSchema = _joi2.default.array().items(_joi2.default.string().required());
